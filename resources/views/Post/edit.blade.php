@extends('panelbase')
    ...
    @section('content')
      ...
      <hr>
      {!! Form::model($post, ['method' => 'POST', 'url' => route('updatepost', $post->slug), 'class' => 'mb-5', 'enctype' => "multipart/form-data"]) !!}
          ...
          <div class="row">
              <div class="col-4">
                  <div class="form-group">
                      ...
                      {{ Form::select('category_id', $categories, null, ['class' => 'form-control form-control-sm', 'required']) }}
                      @error('category')
                        <small class="text-danger">{{ $message }}</small>
                      @enderror
                  </div>
